	...
	html_content += '''
		...
		function packOnePickOne() {
			...
			canvas.addEventListener("mousemove", function(e) {
				...
				// Calculate which card is being hovered (5 columns, each card is 375x523)
				const col = Math.floor(x / 377);
				const row = Math.floor(y / 525);
				const index = row * 5 + col;
				
				// Check if we're within a valid card position
				if (index >= 0 && index < img_list.length && col < 5 && row < 3) {
					const cardX = col * 377;
					const cardY = row * 525;
